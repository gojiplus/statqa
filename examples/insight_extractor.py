...
Features:
  ...
  1. Descriptive analyses:
     ...
     - Bivariate: Pearson correlations (num x num), group means (cat x num); skip weight variables or single-level
  2. Interpretation: clear natural-language insights with metadata labels, missing info, and no-weight disclaimers
  3. Visualization: consistent small (6 x 4) matplotlib charts saved within output-dir
...
def univariate_analysis(df, var, vtype, labels, missing_map, valid_map, output_dir):
    ...
    ser = df[var].replace(dict.fromkeys(missing_map.get(var, []), np.nan))
    fig_path = None
    if vtype == "numeric":
        ...
        fig.text(
            0.5, 0.01, f"N={len(data)}, dropped {miss} missing; no weights", ha="center", fontsize=8
        )
        fig_path = os.path.join(output_dir, f"univariate_{var}.png")
        fig.savefig(fig_path, bbox_inches="tight")
        plt.close(fig)
        txt = (
            f"**{pretty}** ({var}): mean={mean_:.2f}, median={med:.2f}, std={std:.2f}. "
        ...
        ax.set_ylabel("Percentage (%)")
        fig.text(
            0.5, 0.01, f"N={total}, dropped {miss} missing; no weights", ha="center", fontsize=8
        )
        fig_path = os.path.join(output_dir, f"univariate_{var}.png")
        fig.savefig(fig_path, bbox_inches="tight")
        plt.close(fig)
        ...
        txt = (
            f"**{pretty}** ({var}): top value={top} ('{desc}') at {pct.max():.1f}%. "
...
def bivariate_analysis(df, x, y, types, labels, missing_map, valid_map, output_dir):
    ...
    if types.get(x) == "categorical" and types.get(y) == "numeric":
        ...
        txt = (
            f"Mean **{labels.get(y,y)}** by **{labels.get(x,x)}**: {mapping}. "
...
def main():
    ...
    for i in tqdm(range(len(vars50)), desc="Bivariate", unit="pair"):
        for j in range(i + 1, len(vars50)):
            ins = bivariate_analysis(
                df, vars50[i], vars50[j], types, labels, missing_map, valid_map, args.output_dir
            )
            if ins:
                insights.append(ins)

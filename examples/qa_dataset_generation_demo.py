...
def create_survey_dataset() -> tuple[pd.DataFrame, Codebook]:
    ...
    data = pd.DataFrame({
        ...
        'education': np.random.choice(['High School', 'Bachelor', 'Graduate'], n, p=[0.4, 0.4, 0.2]),
...
def run_pipeline_on_dataset(
    ...
    with open(dataset_dir / 'codebook.json', 'w') as f:
        json.dump({var.name: var.model_dump() for var in codebook.variables.values()}, f, indent=2, default=str)

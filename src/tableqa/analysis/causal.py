...
class CausalAnalyzer:
    ...
    def _linear_regression(
        ...
        # Prepare variables
        y = data[outcome_name]
        X_vars = [treatment_name]  # noqa: N806
        if control_vars:
            X_vars.extend([v.name for v in control_vars])

        X = data[X_vars]  # noqa: N806
        X = sm.add_constant(X)  # Add intercept  # noqa: N806
    ...
    def _sensitivity_analysis(
        ...
        y = data[outcome_name]
        X_no_controls = sm.add_constant(data[[treatment_name]])  # noqa: N806
        model_no_controls = sm.OLS(y, X_no_controls).fit()

        # Model with controls
        X_vars = [treatment_name] + [v.name for v in control_vars]  # noqa: N806
        X_with_controls = sm.add_constant(data[X_vars])  # noqa: N806
        ...
        return {
            ...
            "confounding": {
                ...
                "substantial": bool(abs(percent_change) > 10)
